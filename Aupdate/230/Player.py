    ...
    def play_thread(self, task):
        ...
        if Constant.PAUSE_FLAG:
            ...
            if Constant.PAUSE_FLAG:
                ...
                print("播放结束，检测流是否存在")
                while RTSPCheck.send_main(task.get("streaming_url")):
                    print("流还在，继续播放")
                    cmd = "mplayer -af volume=" + volume + " -rtsp-stream-over-tcp -vo null " + task.get("streaming_url")
                    popen = subprocess.Popen(cmd, shell=True, stdin=subprocess.PIPE)
                    popen.wait()
                    print("播放结束，检测流是否存在")
                    if Constant.PAUSE_FLAG or Constant.CANCLE_FLAG:
                        print("被打断，无需检测流是否还存在")
                        break
                print("流不存在了,或者被打断了")
                if not Constant.PAUSE_FLAG:
                    try:
                        print("不是被打断，移除任务")
                        self.task_array.remove(task)
                    except Exception as e:
